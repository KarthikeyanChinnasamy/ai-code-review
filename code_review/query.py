...
api_key = config.get('config', 'openai_api_key')
...
#         logging.error(f"File not found: {code_file}")
#         return {
#             "code_file": code_file,
#             "code_snippet": "",
...
#         analysis = get_code_analysis(code_content)
#     except Exception as e:
#         logging.error(f"Error analyzing code file: {code_file}")
#         analysis = f"Error analyzing code file: {e}"
...
def get_num_tokens_from_messages(messages, model="gpt-4o-mini"):
    ...
    if model == "gpt-3.5-turbo":
        logging.debug(
            "gpt-3.5-turbo may change over time. Returning num tokens assuming gpt-4o-mini."
        )
        ...
        tokens_per_message = 2  # Example assumption
        tokens_per_name = 0     # Example assumption
    else:
...
#     prompt = dedent(f"""
# You are an expert developer and git super user. You do code reviews based on the code from the repository. Complete the following tasks, and be extremely critical and precise in your review:
...
# This is the code from the repository: \n\n {code}
...
# """)
    ...
    # print("Tokens in messages:", tokens_in_messages)
    max_tokens = 4096
    tokens_for_response = max_tokens - tokens_in_messages
    # print("Tokens for response:", tokens_for_response)
